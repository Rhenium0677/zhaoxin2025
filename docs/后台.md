...
DELETE {baseurl}/api/admin/stu/{id}
...
ID 或 NetID 必须传入一个，若两个都传入则以 ID 为准。但是不建议依赖 NetID 进行更新，因为不排除小东西卡出 Bug 会出现多个面试。

抽题完成仍会更新 status 为面试中 1。

状态改成未面试需要传 3，但是数据库中实际存储的仍是未面试 0 面试中 1 已面试 2。Pass 同理，传 1 表示通过，传 2 表示未通过，实际存储的仍是通过 1 未通过 0。
...
|status|body| integer | 否  | 面试中:1 已面试:2 未面试:3   |
