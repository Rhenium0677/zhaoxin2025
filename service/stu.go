...
func (*Stu) GetInterv(date time.Time) ([]model.Interv, error) {
	...
	if err := model.DB.Where("time BETWEEN ? AND ?", timeRange.Start, timeRange.End).Where("time > ?", time.Now().Add(1 * time.Hour)).Find(&data).Error; err != nil {
...
func (*Stu) CancelInterv(netid string, intervid int) error {
	...
	// 取消学生的面试预约
	if err := model.DB.Model(&model.Interv{}).Where("id = ?", intervid).Update("netid", nil).Update("department", model.None).Error; err != nil {
